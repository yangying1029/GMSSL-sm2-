...
+ (NSArray<NSString *> *)createKeyPair{
    ...
    EC_GROUP *group = EC_GROUP_new_by_curve_name(NID_sm2p256v1); // 椭圆曲线
    EC_KEY *key = NULL; // 密钥对
    do {
        key = EC_KEY_new();
...
//        if (!SM2_ciphertext_size(key, ctext_len)) {
//            break;
//        }else {
//           ctext_len = SM2_ciphertext_size(key, ctext_len);
//        }
        
        
        if (!sm2_ciphertext_size_yy(key, digest, msg_len, &ctext_len)) {
            break;
        }
        ...
        if (!SM2_encrypt(NID_sm2p256v1, plain_bytes, msg_len, ctext, &ctext_len, key)) {
            break;
        }
...
+ (nullable NSData *)deData:(NSData *)cipherData hexPriKey:(NSString *)hexPriKey{
    uint8_t *cipher_bytes = (uint8_t *)cipherData.bytes; // 加密后的明文
    ...
    do {
        ...
        if (!SM2_decrypt(NID_sm2p256v1, cipher_bytes, ctext_len, plaintext, &ptext_len, key)) {
            break;
        }
